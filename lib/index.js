...
export function apply(ctx) {
  ...
  try {
    ctx.tools.register(makeTool(
      'maclens_ocr',
      'Run Apple on-device Vision OCR on an image (local file path). Returns every visible text line with confidence and bounding box. Use for screenshots, scans, photos with text, or any image whose text the current model cannot read directly. Enable "slice" for very tall screenshots (long chat logs, scrolling captures) — Vision misses small text on tall images without it.',
      {
        ...
        slice: { type: 'boolean', description: 'Slice tall images into overlapping strips for reliable OCR (recommended for screenshots taller than ~4000px)' },
        sliceHeight: { type: 'number', description: 'Strip height in pixels when slicing (default 4096)' },
      },
      ['path'],
      null,
      'ocr',
  ...
  try {
    ctx.tools.register(makeTool(
      ...
      'Parse a document page with Apple on-device Vision (local file path): OCR text plus a simple column layout analysis. Use for scanned pages, PDF renders, or multi-column documents.',
      {
        ...
        languages: { type: 'string', description: 'Comma-separated recognition languages (default: zh-Hans,en-US)' },
        maxLines: { type: 'number', description: 'Cap the number of OCR lines returned (for very tall screenshots)' },
        slice: { type: 'boolean', description: 'Slice tall images into overlapping strips for reliable OCR' },
        sliceHeight: { type: 'number', description: 'Strip height in pixels when slicing (default 4096)' },
      },
      ['path'],
      null,
